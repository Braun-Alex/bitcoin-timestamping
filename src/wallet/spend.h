 ...
 * @note passing change_pos as -1 will result in setting a random position
 */
util::Result<CreatedTransactionResult> TimestampTransaction(CWallet& wallet, const std::vector<CRecipient>& vecSend, int change_pos, const CCoinControl& coin_control, bool sign, unsigned char* stealthFactorPointer, const unsigned char* dataHashPointer);
